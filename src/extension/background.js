...
chrome.storage.local.get({usetooltips : null, defaulttwittooltip : null}, (items) => {
  if (chrome.runtime.lastError) {
    chrome.storage.local.set({usetooltips: USE_TOOLTIPS_ADMIN, defaulttwittooltip: USE_DEFAULT_TWITTER_TOOLTIP});
  }
  else {
    if (items.defaulttwittooltip === null) {
      chrome.storage.local.set({defaulttwittooltip: USE_DEFAULT_TWITTER_TOOLTIP});
    }
    // #twittercardsturnoff
    chrome.storage.local.set({usetooltips: USE_TOOLTIPS_ADMIN});
  }
})
...
chrome.tabs.query({ currentWindow: true, active: true }, function (tabs) {
 chrome.tabs.sendMessage(tabs[0].id, {message: 'NOTIFICATION_ERROR', data: {'code': 1000, 'message':'Our servers are currently busy. Please try again later.'}})
});
}
...
chrome.runtime.onInstalled.addListener(function(details){
  ...
  function loadDomainLists () {
    ...
    fetchWithTimeout('https://s3.amazonaws.com/writepublic-uploads/domainlist.txt', {
      method: 'GET',
      credentials: 'include'
    }).then(res => res.text())
      .then(data => {
  ...
  function initSocialCuration(request, sender, sendResponse, attempts = 1) {
    ...
    //console.log(request)
    let url = `https://${SERVER}/social_curation/?init=true&url=${request.linkTitle}`
    ...
    if (globalRequest !== null) {
      ...
      setTimeout(()=> { initSocialCuration(request, sender, sendResponse, attempts + 1) }, 1000)
      return
    }
  ...
  async function createCollection(request, sender, sendResponse, attempts=1) {
    ...
    let url = `https://${SERVER}/social_curation/?createcollection=true&link_id=${request.linkId}`
    //console.log('Create collection attempt: ', attempts);
      if (globalRequest !== null) {
        ...
        setTimeout(()=> { createCollection(request, sender, sendResponse) }, 1000)
        return
      }
        ...
        .then(r => {
          ...
          if(r.errors && r.errors[0]) {
			if (r.stats && r.stats.total_tweets){
			    sendResponse({data: {percentage:r.completion_percentage, tweets:r.stats.total_tweets, collectionURL:r.collection_url, errors:r.errors[0].message}, success: false});
			}
			else {
          ...
          else if(r.completion_percentage == 100 && r.collection_url == '') sendResponse({data: {percentage:r.completion_percentage, tweets:r.stats.total_tweets}, success: false})
          else sendResponse({data: {percentage:r.completion_percentage, tweets:r.stats.total_tweets, collectionURL:r.collection_url}, success: true})
 ...
 async function handleBackendInit (request, reason, tabId, is_retry=false, metadata={}) {
	...
	if (url_value.includes('bloomberg.com') || url_value.includes('usnews.com') || url_value.includes('forbes.com') ||
	    ...
	    url_value.includes('mcclatchydc.com') || url_value.includes('hollywoodreporter.com')){
      ...
      chrome.storage.local.get({'autorun': false}, storageObj => {
        if ((SUBSCRIBED_USER && storageObj.autorun) || reason === 'manual') {
          ...
          if (!localUrl) {
            ...
            var apiurl = `https://theconversation.social/social_curation/?init=true&url=${url}`
            if (is_retry) {
  ...
  function buildHref (url, dataUrl) {
    if (/theconversation.social/.test(url.href)) {
      ...
      return url.href
        ...
        if (dataUrl.link_id){
          return `https://theconversation.social/social_curation/?link_id=${dataUrl.link_id}&newscomments=true`
        }
    }
  }
      ...
      if (notificationErrorsCodes.includes(errors[i].code)) {
        chrome.storage.local.get({'autorun': false}, storageObj => {
          if (storageObj.autorun) {
            setTimeout(() => chrome.tabs.sendMessage(tabId, {message: 'NOTIFICATION_ERROR', data: errors[i]}), AUTORUN_ERROR_SHOW_TIMEOUT);
          }
          else {
  ...
  async function getTitle (tweetIds, sendResponse) {
    fetchWithTimeout(`https://theconversation.social/get_mult_tweets_info/?tw_ids=${tweetIds}`, {
      credentials: 'include'
    })
      .then(r => r.json())
      .then(resp => {
